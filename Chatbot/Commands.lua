...
local function RegisterCommand(module, name, callbackName)
    Addon.Trace("RegisterCommand(%s, %s, %s)", module.name, name, callbackName)

    if not AceConsole.commands[name] then
        module:RegisterChatCommand(name, callbackName)
        Addon.Debug("Command /%s is now registered to %s.", name, callbackName)
    else
        Addon.Trace("Command /%s is was already registered to %s.", name, callbackName)
    end
end

local function UnregisterCommand(module, name, callbackName)
    Addon.Trace("UnregisterCommand(%s, %s, %s)", module.name, name, callbackName)

    if AceConsole.commands[name] then
        module:UnregisterChatCommand(name, callbackName)
        Addon.Debug("Command /%s is now unregistered from %s.", name, callbackName)
    else
        Addon.Trace("Command /%s is was already unregistered from %s.", name, callbackName)
    end
end

local function ToggleCommand(module, name, callbackName, enabled)
    Addon.Trace("ToggleCommand(%s, %s, %s, %s)", module.name, name, callbackName, tostring(enabled or "nil"))
...
local function ToggleAlias(module, name, callbackName, enabled)
    Addon.Trace("ToggleAlias(%s, %s, %s, %s)", module.name, name, callbackName, tostring(enabled))
...
function Addon:SetupModuleCommands(name, module, enabled)
    Addon.Trace("Addon:SetupModuleCommands(%s, %s, %s)", name, module ~= nil and module.name or "nil",
